 ...
 */
const generateRoutes = function(appName, routes, name){
    let modelName = upperFirstLetter(name);
    let data = `var express = require('express');\n`;
    data += `var router = express.Router()\n`;
    data += `var ${modelName} = require('../models/${name}');\n`;
    data += `\n`;
    ...
    fileWriter(`./${appName}/routes/${name}.js`, data);
}

const generateRoute = function(appName, models){
    models.forEach(model => {
        generateRoutes(appName, model.methods, model.name);
    });
}
