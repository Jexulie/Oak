...
const generateSchemaItem = function(name, type, isRequired, defaultValue=undefined){
    ...
    if(isRequired === true) row += `, required: ${isRequired}`;
...
const generateGetByID = function(model){
    ...
    data += `${modelName}.findById(id)\n\t\t\t`;
    data += `.then(resp => {\n\t\t\t\t`;
    data += `resolve({success: true, payload: resp});\n\t\t\t`;
    data += `})\n\t\t\t`;

    data += `.catch(error => {\n\t\t\t\t`;
    data += `${modelName}Logger.error('get${modelName}byID Error: ' + error);\n\t\t\t\t`;
    data += `reject({success: false, error});\n\t\t\t`;
    data += `});\n\t\t`;
    data += `});\n`;
    data += `}\n`;
    data += '\n';
...
const generateRemove = function(model){
    ...
    data += `.then(resp => {\n\t\t\t\t`;
    data += `resolve({success: true, payload: null});\n\t\t\t`;
    data += `})\n\t\t\t`;
...
const generateModels = function(models){
    models.forEach(model => {
        let data;
        if(!model.isSQL){
            data = generateNoSQL(model);
            filewriter(`./${model.name}.js`, data);
        }else{
...
//         name: 'books',
//         isSQL: false,
//         methods: ['add', 'get', 'modify', 'update', 'remove'],
//         schema: [
